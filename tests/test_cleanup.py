...
def test_cleanup_deletes_old_mails(
    mock_imap, mock_tqdm, mock_console, fake_config, monkeypatch
):
    ...
    new_date = (datetime.now(timezone.utc) - timedelta(days=10)).strftime(
        "%a, %d %b %Y %H:%M:%S +0000"
    )
