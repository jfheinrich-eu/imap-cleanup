...
def test_get_imap_folders_success():
    ...
    mock_imap_instance.list.return_value = (
        'OK', [b'() "/" "INBOX"', b'() "/" "Archive"'])
    mock_imap_instance.logout.return_value = ('OK', [])
    folders = cli.get_imap_folders(
        'server', 993, 'user', 'pw', True, imap_class_ssl=mock_imap, imap_class=mock_imap)
...
def test_get_imap_folders_error():
    ...
    with pytest.raises(Exception):
        cli.get_imap_folders('server', 993, 'user', 'pw', True,
                             imap_class_ssl=mock_imap, imap_class=mock_imap)
