...
def cleanup():
    if not os.path.exists(CONFIG_FILE):
        console.print(
            f"[red]Config file {CONFIG_FILE} not found. Please run init first.[/red]"
        )
        return
    ...
    for accounts in config.get("accounts", []):
        ...
        for folder, opts in folders.items():
            ...
            try:
                ...
                with tqdm(
                    total=len(msg_nums), desc=f"{user}:{folder}", unit="mail"
                ) as pbar:
                    for num in msg_nums:
                        typ, msg_data = M.fetch(
                            num, "(BODY.PEEK[HEADER.FIELDS (DATE)])"
                        )
                        if typ != "OK":
                            pbar.update(1)
                            continue
...
def get_imap_folders(
    ...
    password,
    ssl,
    imap_class_ssl=imaplib.IMAP4_SSL,
...
def init_config(
    ...
    folder_config = {
        folder: {"purge": False, "retention_days": 30} for folder in selected_folders
    }
    config = {
        "accounts": [
            {
...
def main():
    ...
    parser_cleanup = subparsers.add_parser(
        "cleanup", help="Cleanup emails according to config"
    )
